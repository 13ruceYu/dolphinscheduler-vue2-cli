<template>
  <div class="clearfix dag-model">
    ...
    <div class="dag-contect">
      ...
      <el-drawer :visible.sync="nodeDrawer" size="" :with-header="false">
        <m-form-model
          v-if="nodeDrawer"
          :nodeData="nodeData"
      ...
      <el-drawer :visible.sync="lineDrawer" size="" :wrapperClosable="false" :with-header="false">
        <m-form-line-model :lineData="lineData" @addLineInfo="addLineInfo" @cancel="cancel"></m-form-line-model>
      </el-drawer>
      <el-drawer :visible.sync="udpDrawer" size="" :wrapperClosable="false" :with-header="false">
        <m-udp></m-udp>
      </el-drawer>
      <el-dialog :title="$t('Set the DAG diagram name')" :visible.sync="dialogVisible" width="auto">
        <m-udp @onUdp="onUdpDialog" @close="closeDialog"></m-udp>
      </el-dialog>
      <el-dialog :title="$t('Please set the parameters before starting')" :visible.sync="startDialog" width="auto">
        <m-start
          :startData="startData"
      ...
      return new Promise((resolve) => {
        this.getTaskState(this.urlParam.id).then((res) => {
          ...
          const titleTpl = (item, desc) => {
            ...
            return `<div style="text-align: left">${i18n.$t('Name')}：${$item.name}</br>${i18n.$t(
              'State'
            )}：${desc}</br>${i18n.$t('type')}：${$item.taskType}</br>${i18n.$t('host')}：${
              $item.host || '-'
            }</br>${i18n.$t('Retry Count')}：${$item.retryTimes}</br>${i18n.$t('Submit Time')}：${formatDate(
              $item.submitTime
            )}</br>${i18n.$t('Start Time')}：${formatDate($item.startTime)}</br>${i18n.$t('End Time')}：${
          ...
          data.forEach((v1) => {
            idArr.forEach((v2) => {
              if (v2.name === v1.name) {
                ...
                state.append(
                  `<strong class="${v1.icoUnicode} ${v1.isSpin ? 'as as-spin' : ''}" style="color:${
                    v1.color
        ...
        Dag.saveStore().then((res) => {
          if (this._verifConditions(res.tasks)) {
            if (this.urlParam.id) {
               ...
               */
              this[this.type === 'instance' ? 'updateInstance' : 'updateDefinition'](this.urlParam.id)
                .then((res) => {
                  ...
                  if (this.type === 'instance') {
                    this.$router.push({
                      path: `/projects/instance/list/${this.urlParam.id}?_t=${new Date().getTime()}`,
                    })
                  } else {
                    this.$router.push({
                      path: `/projects/definition/list/${this.urlParam.id}?_t=${new Date().getTime()}`,
                    })
                  }
                  resolve()
                })
              ...
              this.saveDAGchart()
                .then((res) => {
                  this.$message.success(res.msg)
                  this.spinnerLoading = false
                  // source @/view/dag/_source/editAffirmModel/index.js
      ...
      tasks.map((v) => {
        if (
          ...
          (v.conditionResult.successNode[0] === '' ||
            ...
            v.conditionResult.failedNode[0] === '' ||
            v.conditionResult.failedNode[0] === null)
        ) {
          bool = false
          return false
        }
