  ...
  <div class="form-model-wrapper" v-clickoutside="_handleClose">
        ...
        <m-log :item="backfillItem">
          <template slot="history"
            ><a href="javascript:" @click="_seeHistory"
              ><em class="ansicon el-icon-alarm-clock"></em><em>{{ $t('View history') }}</em></a
            ></template
          >
          <template slot="log"
            ><a href="javascript:"
              ><em class="ansicon el-icon-document"></em><em>{{ $t('View log') }}</em></a
            ></template
          >
        </m-log>
        <a href="javascript:" @click="_goSubProcess" v-if="_isGoSubProcess"
          ><em class="ansicon ri-node-tree"></em><em>{{ $t('Enter this child node') }}</em></a
        >
      </span>
    </div>
        ...
        <div class="clearfix list" v-if="nodeData.taskType === 'CONDITIONS'">
          ...
          <div class="cont-box">
            ...
            <el-select style="width: 157px" size="small" v-model="successBranch" clearable>
              <el-option
                v-for="item in nodeData.rearList"
                :key="item.value"
                :value="item.value"
                :label="item.label"
              ></el-option>
    ...
    <div class="bottom-box">
      <div class="submit" style="background: #fff">
        ...
        <el-button type="primary" size="small" round :loading="spinnerLoading" @click="ok()" :disabled="isDetails"
...
import Conditions from './tasks/conditions'
...
import mPriority from '@/components/priority/priority'
  ...
  methods: {
    ...
    _goSubProcess() {
      if (_.isEmpty(this.backfillItem)) {
        this.$message.warning(
          `${i18n.$t('The newly created sub-Process has not yet been executed and cannot enter the sub-Process')}`
        )
        return
      }
      if (this.router.history.current.name === 'projects-instance-details') {
        ...
        this.store
          .dispatch('dag/getSubProcessId', { taskId: stateId })
          .then((res) => {
    ...
    _verifName() {
      ...
      if (this.successBranch !== '' && this.successBranch !== null && this.successBranch === this.failedBranch) {
        this.$message.warning(
          `${i18n.$t('Cannot select the same node for successful branch flow and failed branch flow')}`
        )
        return false
      }
    ...
    _verifWorkGroup() {
      ...
      if (item === undefined) {
        this.$message.warning(
          `${i18n.$t('The Worker group no longer exists, please select the correct Worker group!')}`
        )
        return false
      }
      return true
      ...
      if (this.$refs.PRE_TASK) {
        if (!this.$refs.PRE_TASK._verification()) {
          ...
          $(`#${this.nodeData.id}`).attr(
            'data-targetarr',
            this.preTaskIdsInWorkflow ? this.preTaskIdsInWorkflow.join(',') : ''
          )
          ...
          for (let i = 0; i < len; i++) {
            if (
              this.preTasksToDelete.indexOf(currentConnects[i].sourceId) > -1 &&
              currentConnects[i].targetId === targetId
            ) {
