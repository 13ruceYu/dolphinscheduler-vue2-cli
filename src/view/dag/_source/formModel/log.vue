    ...
    <transition name="fade">
      <div v-show="isLog || source === 'list'" class="log-pop">
        <div class="log-box">
          ...
          <div class="operation">
            <el-button type="primary" size="small" round @click="close">
              {{ $t('Close') }}
            </el-button>
          </div>
        </div>
...
const handerTextareaSize = (isH = 0) => {
  ...
  return $('.textarea-ft').css({
    height: `${$('.content-log-box').height() - isH}px`,
  })
}
...
export default {
  ...
  methods: {
    _refreshLog() {
      this.loading = true
      this.store
        .dispatch('dag/getLog', this._rtParam)
        .then((res) => {
          setTimeout(() => {
            this.loading = false
            if (res.data) {
