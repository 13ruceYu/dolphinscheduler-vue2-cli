      ...
      this._getProcessByProjectId(projectId).then((definitionList) => {
        ...
        this._getDependItemList(val).then((depTasksList) => {
          this.$nextTick(() => {
            this.$emit(
              'dependItemListEvent',
              _.concat(this.dependItemList, this._rtNewParams(val, definitionList, depTasksList, projectId))
            )
          })
        })
      })
      ...
      if (!this.dependItemList.length) {
        ...
        this._getProcessByProjectId(projectId).then((definitionList) => {
          ...
          this._getDependItemList(value).then((depTasksList) => {
            this.$emit(
              'dependItemListEvent',
              _.concat(this.dependItemList, this._rtNewParams(value, definitionList, depTasksList, projectId))
            )
          })
        })
      } else {
