...
<template>
  <div class="datasource-model">
    <div class="select-listpp">
      <el-select v-model="type" style="width: 160px" size="small" @change="_handleTypeChanged" :disabled="isDetails">
        <el-option v-for="city in typeList" :key="city.code" :value="city.code" :label="city.code"> </el-option>
      </el-select>
      <el-select
        :placeholder="$t('Please select the datasource')"
        v-model="datasource"
        style="width: 288px"
        size="small"
        :disabled="isDetails"
      >
        <el-option v-for="city in datasourceList" :key="city.id" :value="city.id" :label="city.code"> </el-option>
      </el-select>
    </div>
  </div>
</template>
