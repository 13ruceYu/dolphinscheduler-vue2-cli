...
<template>
  <div class="user-def-params-model">
    <div class="select-listpp" v-for="(item, $index) in localParamsList" :key="item.id" @click="_getIndex($index)">
      <el-input
        :disabled="isDetails"
        type="text"
        size="small"
      ...
      <template v-if="hide">
        <el-select
          ...
          :disabled="isDetails"
        >
          <el-option v-for="city in directList" :key="city.code" :value="city.code" :label="city.code"> </el-option>
        </el-select>
        <el-select
          style="width: 118px"
          size="small"
