...
<template>
  <div class="user-def-params-model">
    <div class="select-listpp" v-for="(item, $index) in httpParamsList" :key="item.id" @click="_getIndex($index)">
      <el-input
        :disabled="isDetails"
        type="text"
        size="small"
      ...
      <el-select
        ...
        :style="inputStyle"
      >
        <el-option v-for="position in positionList" :key="position.code" :value="position.id" :label="position.code">
        </el-option>
      </el-select>
      <el-input
        :disabled="isDetails"
