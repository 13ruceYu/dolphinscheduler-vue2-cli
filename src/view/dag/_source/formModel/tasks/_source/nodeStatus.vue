  ...
  <div class="dep-list-model">
    <div v-for="(el, $index) in dependItemList" :key="$index" class="list" @click="itemIndex = $index">
      <el-select style="width: 150px" size="small" v-model="el.depTasks" :disabled="isDetails">
        <el-option v-for="item in preNode" :key="item.value" :value="item.value" :label="item.label"> </el-option>
      </el-select>
      ...
      <template v-if="isInstance">
        <span class="instance-state">
          ...
          <em
            ...
            :class="'icon-' + el.state"
            v-if="el.state === 'RUNNING_EXECUTION'"
            data-toggle="tooltip"
            data-container="body"
            :title="$t('waiting')"
          ></em>
          ...
          _.map(this.dependItemList, (v, i) => {
            this._getProcessByProjectId(v.projectId).then(() => {
              this.$set(
                this.dependItemList,
                i,
                this._rtOldParams(v.definitionId, ['ALL'].concat(_.map(res[v.definitionId] || [], (v) => v.name)), v)
              )
            })
          })
        })
