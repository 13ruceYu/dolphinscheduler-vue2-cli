...
<template>
  <div class="spark-model">
    <m-list-box>
      ...
      <div slot="content">
        <el-select style="width: 130px" size="small" v-model="ruleId" :disabled="isDetails" @change="_handleRuleChange">
          <el-option v-for="rule in ruleNameList" :key="rule.value" :value="rule.value" :label="rule.label">
          </el-option>
        </el-select>
      </div>
    </m-list-box>
  ...
  methods: {
    ...
    srcConnectorTypeChange() {
      this._updateSelectFieldOptions(
        ...
        'src_datasource_id',
        ['src_datasource_id', 'src_table', 'src_field']
      )
    },
    ...
    targetConnectorTypeChange() {
      this._updateSelectFieldOptions(
        ...
        ['target_datasource_id', 'target_table', 'target_field']
      )
    },
