...
<template>
  <div class="http-model">
    ...
    <m-list-box>
      ...
      <div slot="content">
        <el-select style="width: 150px" size="small" v-model="httpMethod" :disabled="isDetails">
          <el-option v-for="city in httpMethodList" :key="city.code" :value="city.code" :label="city.code"> </el-option>
        </el-select>
      </div>
    </m-list-box>
    <m-list-box>
      ...
      <div slot="content">
        <m-http-params ref="refHttpParams" @on-http-params="_onHttpParams" :udp-list="httpParams" :hide="false">
        </m-http-params>
      </div>
    </m-list-box>
    <m-list-box>
      ...
      <div slot="content">
        <el-select style="width: 230px" size="small" v-model="httpCheckCondition" :disabled="isDetails">
          <el-option v-for="city in httpCheckConditionList" :key="city.code" :value="city.code" :label="city.value">
          </el-option>
        </el-select>
      </div>
    </m-list-box>
    ...
    return {
      ...
      httpCheckConditionList:
        cookies.get('language') === 'en_US'
          ? [
              { code: 'STATUS_CODE_DEFAULT', value: 'Default response code 200' },
              { code: 'STATUS_CODE_CUSTOM', value: 'Custom response code' },
              { code: 'BODY_CONTAINS', value: 'Content includes' },
              { code: 'BODY_NOT_CONTAINS', value: 'Content does not contain' },
            ]
          : [
              { code: 'STATUS_CODE_DEFAULT', value: '默认响应码200' },
              { code: 'STATUS_CODE_CUSTOM', value: '自定义响应码' },
              { code: 'BODY_CONTAINS', value: '内容包含' },
              { code: 'BODY_NOT_CONTAINS', value: '内容不包含' },
            ],
    }
  },
  props: {
