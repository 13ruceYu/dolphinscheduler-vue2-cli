...
import { uuid, findComponentDownward } from '@/util/'

import { tasksAll, rtTasksTpl, setSvgColor, saveTargetarr, rtTargetarrArr, computeScale } from './util'
...
JSP.prototype.init = function ({ dag, instance, options }) {
  ...
  this.JspInstance.bind('click', (e) => {
    ...
    if (this.config.isClick) {
      ...
      findComponentDownward(this.dag.$root, 'dag-chart')._createLineLabel({
        id: e._jsPlumb.overlays.label.canvas.id,
        sourceId: e.sourceId,
        targetId: e.targetId,
      })
    }
  ...
  if (this.config.isContextmenu) {
    ...
    const html = [
      `<a href="javascript:" id="startRunning" class="${
        isOne ? '' : 'disbled'
      }"><em class="el-icon-video-play"></em><span>${i18n.$t('Start')}</span></a>`,
      `<a href="javascript:" id="editNodes" class="${
        isTwo ? 'disbled' : ''
      }"><em class="el-icon-edit-outline"></em><span>${i18n.$t('Edit')}</span></a>`,
      `<a href="javascript:" id="copyNodes" class="${
        isTwo ? 'disbled' : ''
      }"><em class="el-icon-copy-document"></em><span>${i18n.$t('Copy')}</span></a>`,
      `<a href="javascript:" id="removeNodes" class="${
        isTwo ? 'disbled' : ''
      }"><em class="el-icon-delete"></em><span>${i18n.$t('Delete')}</span></a>`,
    ]
...
JSP.prototype.saveStore = function () {
  return new Promise((resolve, reject) => {
    ...
    if (store.state.dag.connects.length === this.JspInstance.getConnections().length) {
      ...
      store.state.dag.connects.length > 0 &&
...
JSP.prototype.jspBackfill = function ({ connects, locations, largeJson }) {
  ...
  this.JspInstance.batch(() => {
    ...
    _.map(connects, (v) => {
      ...
      if (
        $(`#${sourceId}`).attr('data-tasks-type') === 'CONDITIONS' &&
        $(`#${sourceId}`).attr('data-successnode') === $(`#${targetId}`).find('.name-p').text()
      ) {
        ...
        $(`#${sourceId}`).attr('data-tasks-type') === 'CONDITIONS' &&
        $(`#${sourceId}`).attr('data-failednode') === $(`#${targetId}`).find('.name-p').text()
      ) {
        this.JspInstance.connect({
          source: sourceId,
          target: targetId,
