...
<template>
  <div class="list-model">
          ...
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              ...
              <div slot="reference" class="name-wrapper">
                <a href="javascript:" class="links" @click="_go(scope.row)"
                  ><span class="ellipsis">{{ scope.row.processInstanceName }}</span></a
                >
              </div>
            </el-popover>
          </template>
                  ...
                  <el-button
                    ...
                    :disabled="
                      !(
                        ...
                        scope.row.state === 'NEED_FAULT_TOLERANCE' ||
                        scope.row.state === 'KILL'
                      )
                    "
                    @click="_forceSuccess(scope.row)"
                    circle
                  ></el-button>
                </span>
              </el-tooltip>
              <el-tooltip :content="$t('View log')" placement="top" :enterable="false">
                <span
                  ><el-button
                    type="primary"
                    size="mini"
                    :disabled="scope.row.taskType === 'SUB_PROCESS' ? true : false"
                    icon="el-icon-tickets"
                    @click="_refreshLog(scope.row)"
                    circle
                  ></el-button
                ></span>
              </el-tooltip>
    ...
    <el-dialog :show-close="false" :visible.sync="logDialog" width="auto">
...
import mLog from '@/view/dag/_source/formModel/log'
import { tasksState } from '@/view/dag/_source/config'
...
export default {
  ...
  methods: {
    ...
    _rtState(code) {
      ...
      return `<em class="${o.icoUnicode} ${o.isSpin ? 'as as-spin' : ''}" style="color:${
        o.color
    ...
    _forceSuccess(item) {
      this.forceTaskSuccess({ taskInstanceId: item.id })
        .then((res) => {
          if (res.code === 0) {
    ...
    _go(item) {
      this.$router.push({
        path: `/projects/instance/list/${item.processInstanceId}`,
      })
    },
  },
  watch: {
