...
<template>
  <div class="wrap-table">
    <ListConstruction :title="$t('Process Instance')">
      ...
      <template slot="content">
        <template v-if="processInstanceList.length || total > 0">
          <m-list
            :process-instance-list="processInstanceList"
            @on-update="_onUpdate"
...
import mInstanceConditions from '@/view/projects/pages/_source/conditions/instance/processInstance'
...
export default {
  ...
  methods: {
    ...
    _getProcessInstanceListP(flag) {
      this.isLoading = !flag
      this.getProcessInstance(this.searchParams)
        .then((res) => {
  ...
  components: { mList, mInstanceConditions, Spin, ListConstruction, NoData },
}
</script>
