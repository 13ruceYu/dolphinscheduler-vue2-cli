...
Gantt.prototype.init = function ({ el, tasks }) {
  ...
  this.x = d3.time.scale().domain([this.startTimeXAxis, this.endTimeXAxis]).range([0, this.width]).clamp(true)
  ...
  this.yAxis = d3.svg.axis().scale(this.x).orient('left').tickSize(0)
...
Gantt.prototype.drawChart = function () {
  ...
  svg
    .selectAll('.chart')
    .data(this.tasks, (d) => d.startDate + d.taskName + d.endDate)
    .enter()
    .append('rect')
  ...
  svg
    ...
    .attr(
      'transform',
      `rotate(-${
        this.width / ($('.tick').length - 1) > 50 ? 0 : Math.acos(this.width / ($('.tick').length - 1) / 50) * 57
      })`
    )
