...
<template>
  <ListConstruction :title="config.title">
    <template slot="conditions">
      <Conditions @on-query="_onQuery"></Conditions>
    </template>
    <template slot="content">
      <template v-if="taskRecordList.length || total > 0">
        <m-list
          :task-record-list="taskRecordList"
          @on-update="_onUpdate"
...
import Conditions from './_source/conditions'
...
export default {
  ...
  methods: {
    ...
    _getList(flag) {
      this.isLoading = !flag
      this.store
        .dispatch(`dag/${this.config.apiFn}`, this.searchParams)
        .then((res) => {
          this.taskRecordList = []
