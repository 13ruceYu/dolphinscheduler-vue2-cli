 ...
 * tree Expand hidden
 */
Tree.prototype.treeToggles = function (clicked_d) {
  // eslint-disable-line
...
Tree.prototype.treeUpdate = function (source) {
  return new Promise((resolve, reject) => {
    ...
    const height = Math.max(
      500,
      tasks.length * this.config.barHeight + this.config.margin.top + this.config.margin.bottom
    )
    const width =
    ...
    d3.select('svg').transition().duration(this.duration).attr('height', height).attr('width', width)
    ...
    // Update link
    const link = this.svg.selectAll('path.link').data(this.tree.links(tasks), (d) => d.target.id)
