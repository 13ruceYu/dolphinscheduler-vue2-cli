...
<template>
  <div class="datasource-popup-model">
    ...
    <div class="bottom-p">
      <el-button type="text" ize="mini" @click="_close()">
        {{ $t('Cancel') }}
      </el-button>
      <el-button type="success" size="mini" round @click="_testConnect()" :loading="testLoading">{{
        testLoading ? 'Loading...' : $t('Test Connect')
      }}</el-button>
      <el-button type="primary" size="mini" round :loading="spinnerLoading" @click="_ok()"
...
export default {
  ...
  methods: {
    ...
    _submit() {
      ...
      this.store
        .dispatch(`datasource/${this.item ? 'updateDatasource' : 'createDatasources'}`, param)
        .then((res) => {
          this.$message.success(res.msg)
