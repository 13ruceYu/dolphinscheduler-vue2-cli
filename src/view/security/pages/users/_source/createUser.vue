          ...
          <template slot="content">
            <el-select v-model="queueName" style="width: 100%" size="small">
              <el-input
                ...
                readonly
                :placeholder="$t('Please select a queue')"
                :value="selectedModel ? selectedModel.label : ''"
                @on-click-icon.stop="queueName = ''"
              >
                <em
                  slot="suffix"
                  class="el-icon-error"
...
export default {
  ...
  methods: {
    ...
    _verification() {
      ...
      let regPassword =
        /^(?![0-9]+$)(?![a-z]+$)(?![A-Z]+$)(?![`~!@#$%^&*()_\-+=<>?:"{}|,./;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘’，。、]+$)[`~!@#$%^&*()_\-+=<>?:"{}|,./;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘’，。、0-9A-Za-z]{6,22}$/
      ...
      if (this.userPassword !== '' && this.item) {
        if (!regPassword.test(this.userPassword)) {
          this.$message.warning(
            `${i18n.$t(
              'Password consists of at least two combinations of numbers, letters, and characters, and the length is between 6-22'
            )}`
          )
          return false
        }
    ...
    _submit() {
      ...
      this.store
        .dispatch(`security/${this.item ? 'updateUser' : 'createUser'}`, param)
        .then((res) => {
