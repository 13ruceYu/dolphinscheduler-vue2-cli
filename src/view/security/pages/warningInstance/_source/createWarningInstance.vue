        ...
        <m-list-box-f>
          ...
          <template slot="content">
            <el-select
              ...
              v-if="item.id"
            >
              <el-option v-for="items in pluginInstance" :key="items.id" :value="items.id" :label="items.pluginName">
              </el-option>
            </el-select>
...
export default {
  ...
  methods: {
    ...
    changePlugin() {
      this.store
        .dispatch('security/getUiPluginById', {
    ...
    _submit() {
      ...
      this.store
        .dispatch(`security/${this.item ? 'updateAlertPluginInstance' : 'createAlertPluginInstance'}`, param)
        .then((res) => {
