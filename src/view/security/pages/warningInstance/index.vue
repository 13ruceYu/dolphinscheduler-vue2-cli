...
<template>
  <ListConstruction :title="$t('Warning instance manage')">
...
export default {
  ...
  methods: {
    ...
    _create(item) {
      this.getPlugins({ pluginType: 'ALERT' })
        .then((res) => {
    ...
    _getList(flag) {
      ...
      this.isLoading = !flag
      this.queryAlertPluginInstanceListPaging(this.searchParams)
        .then((res) => {
