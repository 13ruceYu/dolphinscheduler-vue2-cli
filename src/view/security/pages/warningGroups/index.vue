...
<template>
  <ListConstruction :title="$t('Warning group manage')">
      ...
      <Conditions @on-conditions="_onConditions">
        <template slot="button-group" v-if="isADMIN">
          ...
          >
            <m-create-warning
              :item="item"
              :allAlertPluginInstance="allAlertPluginInstance"
              @onUpdate="onUpdate"
              @close="close"
            ></m-create-warning>
          </el-dialog>
...
export default {
  ...
  methods: {
    ...
    _create(item) {
      this.queryAllAlertPluginInstance()
        .then((res) => {
    ...
    _getList(flag) {
      ...
      this.isLoading = !flag
      this.queryAlertGroupListPaging(this.searchParams)
        .then((res) => {
