      ...
      <el-radio-group v-model="radioMonth" vertical size="mini">
        ...
        <div class="list-box">
          <el-radio label="specificlMonth">
            ...
            <el-select
              multiple
              :placeholder="$t('请选择具体月数')"
              size="mini"
              v-model="specificMonthVal"
              @change="onspecificlMonths"
            >
              <el-option v-for="item in selectMonthList" :key="item.value" :value="item.value" :label="item.label">
              </el-option>
            </el-select>
          </el-radio>
        </div>
  ...
  methods: {
    ...
    analyticalValue() {
      return new Promise((resolve, reject) => {
        ...
        if (
          ($monthVal.length === 1 && _.isInteger(parseInt($monthVal))) ||
